// -------------------------------------------------------------------------------------------------
// <copyright file="ReadingOrderExtensions.cs" company="Starion Group S.A.">
//
...
namespace Kalliope.Dal
{
    ...
    public static class ReadingOrderExtensions
    {
        ...
        /// <summary>
        /// Updates the Reference properties of the <see cref="ReadingOrder"/> using the data (identifiers) encapsulated in the DTO
        ...
        public static void UpdateReferenceProperties(this Kalliope.Core.ReadingOrder poco, Kalliope.DTO.ReadingOrder dto, ConcurrentDictionary<string, Lazy<Kalliope.Core.ModelThing>> cache)
        {
            if (poco == null)
            {
                throw new ArgumentNullException(nameof(poco), $"the {nameof(poco)} may not be null");
            ...
            var readingsToAdd = dto.Readings.Except(poco.Readings.Select(x => x.Id));
            foreach (var identifier in readingsToAdd)
            {
                if (cache.TryGetValue(identifier, out lazyPoco))
                {
                    var reading = (Reading)lazyPoco.Value;
                    poco.Readings.Add(reading);
                }
            }
