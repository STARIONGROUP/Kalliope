// -------------------------------------------------------------------------------------------------
// <copyright file="ValueRangeExtensions.cs" company="Starion Group S.A.">
//
...
namespace Kalliope.Dal
{
    ...
    /// <summary>
    /// A static class that provides extension methods for the <see cref="ValueRange"/> class
    /// </summary>
    public static class ValueRangeExtensions
    {
        /// <summary>
        /// Updates the value properties of the <see cref="ValueRange"/> by setting the value equal to that of the dto
        /// Removes deleted objects from the reference properties and returns the unique identifiers
        /// of the objects that have been removed from <see cref="AggregationKind.Composite"/> properties
        /// </summary>
        /// <param name="poco">
        /// The <see cref="ValueRange"/> that is to be updated
        /// </param>
        /// <param name="dto">
        /// The DTO that is used to update the <see cref="ValueRange"/> with
        /// </param>
        /// <returns>
        ...
        /// </exception>
        public static IEnumerable<string> UpdateValueAndRemoveDeletedReferenceProperties(this Kalliope.Core.ValueRange poco, Kalliope.DTO.ValueRange dto)
        {
            if (poco == null)
            {
            ...
            poco.InvariantMaxValue = dto.InvariantMaxValue;

            poco.InvariantMinValue = dto.InvariantMinValue;

            poco.MaxInclusion = dto.MaxInclusion;

            poco.MaxValue = dto.MaxValue;

            if (poco.MaxValueMismatchError != null && poco.MaxValueMismatchError.Id != dto.MaxValueMismatchError)
            {
                identifiersOfObjectsToDelete.Add(poco.MaxValueMismatchError.Id);
                poco.MaxValueMismatchError = null;
            }

            poco.MinInclusion = dto.MinInclusion;

            poco.MinValue = dto.MinValue;

            if (poco.MinValueMismatchError != null && poco.MinValueMismatchError.Id != dto.MinValueMismatchError)
            {
                identifiersOfObjectsToDelete.Add(poco.MinValueMismatchError.Id);
                poco.MinValueMismatchError = null;
            }
        ...
        /// <summary>
        /// Updates the Reference properties of the <see cref="ValueRange"/> using the data (identifiers) encapsulated in the DTO
        ...
        public static void UpdateReferenceProperties(this Kalliope.Core.ValueRange poco, Kalliope.DTO.ValueRange dto, ConcurrentDictionary<string, Lazy<Kalliope.Core.ModelThing>> cache)
        {
            if (poco == null)
            {
                throw new ArgumentNullException(nameof(poco), $"the {nameof(poco)} may not be null");
