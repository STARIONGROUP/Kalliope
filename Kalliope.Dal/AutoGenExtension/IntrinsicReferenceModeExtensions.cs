// -------------------------------------------------------------------------------------------------
// <copyright file="IntrinsicReferenceModeExtensions.cs" company="Starion Group S.A.">
//
...
namespace Kalliope.Dal
{
    ...
    /// <summary>
    /// A static class that provides extension methods for the <see cref="IntrinsicReferenceMode"/> class
    /// </summary>
    public static class IntrinsicReferenceModeExtensions
    {
        /// <summary>
        /// Updates the value properties of the <see cref="IntrinsicReferenceMode"/> by setting the value equal to that of the dto
        /// Removes deleted objects from the reference properties and returns the unique identifiers
        /// of the objects that have been removed from <see cref="AggregationKind.Composite"/> properties
        /// </summary>
        /// <param name="poco">
        /// The <see cref="IntrinsicReferenceMode"/> that is to be updated
        /// </param>
        /// <param name="dto">
        /// The DTO that is used to update the <see cref="IntrinsicReferenceMode"/> with
        /// </param>
        /// <returns>
        ...
        /// </exception>
        public static IEnumerable<string> UpdateValueAndRemoveDeletedReferenceProperties(this Kalliope.Core.IntrinsicReferenceMode poco, Kalliope.DTO.IntrinsicReferenceMode dto)
        {
            if (poco == null)
            {
            ...
            if (poco.Kind != null && poco.Kind.Id != dto.Kind)
            {
                poco.Kind = null;
            }
        ...
        /// <summary>
        /// Updates the Reference properties of the <see cref="IntrinsicReferenceMode"/> using the data (identifiers) encapsulated in the DTO
        ...
        public static void UpdateReferenceProperties(this Kalliope.Core.IntrinsicReferenceMode poco, Kalliope.DTO.IntrinsicReferenceMode dto, ConcurrentDictionary<string, Lazy<Kalliope.Core.ModelThing>> cache)
        {
            if (poco == null)
            {
                throw new ArgumentNullException(nameof(poco), $"the {nameof(poco)} may not be null");
            ...
            if (poco.Kind == null && !string.IsNullOrEmpty(dto.Kind) && cache.TryGetValue(dto.Kind, out lazyPoco))
            {
                poco.Kind = (ReferenceModeKind)lazyPoco.Value;
            }
        }
    }
}
